...
Kyverno는 클러스터에 배포되는 리소스를 검증하는 정책 엔진으로 운영합니다. `staging`, `prod` 환경 모두 `kyverno` 네임스페이스에 배포하고, 현재는 `validationAction: Audit`, `failurePolicy: Ignore` 기준으로 위반 리소스를 리포트와 알람으로 추적합니다.
