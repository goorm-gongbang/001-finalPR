...
```mermaid
flowchart TD
    A["이상 징후 발견<br/><br/>Discord 채널 알림 또는 Grafana 등 대시보드 확인"] --> B["클라우드팀 1차 분류"]
    B --> C1["인프라"]
    B --> C2["애플리케이션"]
    B --> C3["보안/감사"]

    C1 --> D1["재배포 / 스케일링 / 노드 교체 / GitOps 재적용"]
    C2 --> D2["커넥션 / 쿼리 최적화 / PITR 가능 여부 점검 / 신규 복원 계획 수립"]
    C3 --> D3["이벤트 소스 확인 / 위반 리소스 확인 / 예외 보관 필요 여부 판단"]
