...
```mermaid
flowchart LR
    PODS["EKS Pods<br/>메트릭·로그·트레이스"] --> STACK["Prometheus · Loki · Tempo<br/>Thanos · Alertmanager"] --> GRAF["Grafana"]
    AWS["CloudWatch · CloudTrail<br/>EventBridge · Lambda"] --> DC(["Discord"])
    POL["Kyverno<br/>Policy Reporter"] --> DC
    STACK --> DC
    GRAF --> OP(["운영자"])

    classDef podsBox fill:#fff8e1,stroke:#d6b656,color:#5a4a1a
    classDef stackBox fill:#e6f4ea,stroke:#82b366,color:#2d5a34
    classDef awsBox fill:#e7f0fb,stroke:#6c8ebf,color:#223b63
    classDef policyBox fill:#ece3f1,stroke:#9673a6,color:#4a2e5f
    classDef sinkBox fill:#fbeaea,stroke:#b85450,color:#6b2a26
    class PODS podsBox
    class STACK,GRAF stackBox
    class AWS awsBox
    class POL policyBox
    class DC,OP sinkBox
```
