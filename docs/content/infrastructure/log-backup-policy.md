...
| **운영 DB 복구 백업** | RDS Automated Backup + PITR, 수동 스냅샷, PostgreSQL SQL dump 백업 | 운영 데이터 복구 |
...
| 저장소 | 저장 대상 | 용도 |
|---|---|---|
| **S3 운영 백업 저장소** | PostgreSQL SQL dump 백업, 운영 로그 백업 | 운영 복구와 단기 재조사 |
...
- PostgreSQL SQL dump 백업은 장기보관, 이관, 수동 복구를 위한 보조 백업입니다.
...
| 구분 | KST | 대상 |
|---|---|---|
| **운영 DB 보조 백업** | 02:00~02:30 | PostgreSQL SQL dump 백업 |
...
| **SQL dump 백업** | 최근 PostgreSQL SQL dump 백업 성공 시각과 오브젝트 생성 여부 |
