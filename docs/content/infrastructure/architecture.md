...
이 페이지에서는 "누가 어떻게 접근하고, 변경 이력이 어떻게 남고, 이상 징후가 어디로 전파되는가"까지를 인프라 운영 범위로 설명합니다. 상세 위협 분석과 보안 룰셋 설명은 보안 섹션에서 분리하는 편이 문서 구조상 더 적절합니다.
...
보안팀이 상세 위협 모델과 방어 정책을 설명한다면, 인프라팀은 이 페이지에서 "그 정책이 실제로 어느 계층에 어떻게 배치되어 운영되는가"를 설명하는 수준까지 다루는 것이 적절합니다.
