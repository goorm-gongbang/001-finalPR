...
부하테스트 결과에 **운영 트래픽**이나 **Rate Limit 차단** 같은 잡음이 섞이지 않도록, 부하테스트 트래픽을 **전용 경로로 완전 격리**했습니다.

- **전용 Istio 라우팅**: 부하테스트 요청만 식별되는 경로(/load-test)로 VirtualService 분리. 운영 엔드포인트와 겹치지 않아 운영 트래픽 집계·SLO에 영향 없이 **부하테스트 트래픽만 관측**.
- **전용 Namespace**: 부하테스트 대상 워크로드를 별도 네임스페이스에 배포. 부하 중에도 운영 Pod의 집계·로그와 완벽히 분리되어 대시보드 오염 없음.
- **Karpenter 부하테스트 전용 NodePool**: 테스트 워크로드를 별도 NodePool로 프로비저닝하여 **운영 워크로드와 노드 자원 경쟁 제거**. Scale-out/Scale-in 특성도 독립적으로 관측 가능해 KEDA/Karpenter 자체 동작 검증에도 활용.

> **왜 이렇게 분리했나**  
> Rate Limit은 운영 트래픽 보호에 반드시 필요하지만, 부하테스트 입장에선 **병목 측정의 왜곡 요인**입니다. **실제 서비스 처리 능력(TPS·응답시간 한계)**과 서비스 간의 병목을 측정.
...
1000vu에서 seat P99만 4.3s로 튀고 나머지 서비스는 정상 구간일 때, 본 대시보드에서 같은 시점의 **HikariCP acquire wait P99**를 확인해 pool 포화가 실 원인임을 판별했습니다.
