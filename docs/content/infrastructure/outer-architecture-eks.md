...
```mermaid
flowchart TB
    USER["사용자 브라우저"] --> R53["Route 53<br/>playball.example.com<br/>(A · AAAA · Alias)"]
    R53 --> CF["CloudFront Distribution<br/>E2XXXXXXXXXX<br/>ACM: *.playball.example.com"]
    CF --> WAF["AWS WAF<br/>WebACL: playball-waf<br/>(Managed Rules + Rate-based)"]

    subgraph VPC["VPC: vpc-0abc123 (10.0.0.0/16)"]
        direction TB
        IGW["Internet Gateway<br/>igw-0xxx"]

        subgraph AZ_A["AZ-a (ap-northeast-2a)"]
            direction TB
            PUB_A["Public Subnet (a)<br/>subnet-0pub-a<br/>10.0.1.0/24<br/>(10.0.1.1–10.0.1.254)"]
            PRI_A["Private-App (a)<br/>subnet-0app-a<br/>10.0.11.0/24<br/>(10.0.11.1–10.0.11.254)"]
            DB_A["Private-Data (a)<br/>subnet-0db-a<br/>10.0.21.0/24<br/>(10.0.21.1–10.0.21.254)"]
            NAT_A["NAT Gateway (a)<br/>nat-0xxx-a<br/>EIP: 3.X.X.X"]
            ALB_A["ALB ENI (a)<br/>10.0.1.10"]
            WK_A["EKS Worker (a)<br/>10.0.11.21, 10.0.11.22 ...<br/>(Karpenter 동적 할당)"]
            RDS_A["RDS Primary<br/>10.0.21.11<br/>postgres.cluster-xxx.rds"]
            REDIS_A["ElastiCache (a)<br/>10.0.21.21<br/>redis-primary.xxx.cache"]
        ...
        subgraph AZ_C["AZ-c (ap-northeast-2c)"]
            direction TB
            PUB_C["Public Subnet (c)<br/>subnet-0pub-c<br/>10.0.2.0/24<br/>(10.0.2.1–10.0.2.254)"]
            PRI_C["Private-App (c)<br/>subnet-0app-c<br/>10.0.12.0/24<br/>(10.0.12.1–10.0.12.254)"]
            DB_C["Private-Data (c)<br/>subnet-0db-c<br/>10.0.22.0/24<br/>(10.0.22.1–10.0.22.254)"]
            NAT_C["NAT Gateway (c)<br/>nat-0xxx-c<br/>EIP: 3.Y.Y.Y"]
            ALB_C["ALB ENI (c)<br/>10.0.2.10"]
            WK_C["EKS Worker (c)<br/>10.0.12.21, 10.0.12.22 ...<br/>(Karpenter 동적 할당)"]
            RDS_C["RDS Standby<br/>10.0.22.11<br/>(Multi-AZ Sync)"]
            REDIS_C["ElastiCache Replica<br/>10.0.22.21"]
        ...
        BASTION["Bastion EC2<br/>10.0.11.5<br/>t4g.nano · SSM Agent"]
        PRI_A --- BASTION

        VPCE["VPC Endpoints<br/>ECR: vpce-0ecr (10.0.11.100, 10.0.12.100)<br/>S3 Gateway: vpce-0s3<br/>Secrets Mgr: vpce-0sm (10.0.11.110, 10.0.12.110)<br/>STS: vpce-0sts (10.0.11.120, 10.0.12.120)"]
        PRI_A --- VPCE
        PRI_C --- VPCE
        ...
        NAT_A -. "아웃바운드" .- PRI_A
        NAT_C -. "아웃바운드" .- PRI_C
        RDS_A <-. "Multi-AZ Sync" .-> RDS_C
        REDIS_A <-. "Async Replica" .-> REDIS_C
    end
    ...
    WK_A -. "ECR Pull / S3 Put / SM Get" .-> VPCE
    WK_C -. "ECR Pull / S3 Put / SM Get" .-> VPCE
```
