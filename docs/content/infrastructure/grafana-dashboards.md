...
| 구분 | 주요 화면 | 목적 |
|---|---|---|
| **인프라 관측** | EKS 클러스터 개요, K8s 운영 현황판, Istio 워크로드 모니터링 | 클러스터 상태와 자원 사용량 확인 |
...
| **비즈니스 관측** | 비즈니스 KPI, 프론트엔드 행동 분석 | 티켓팅 흐름과 전환 지표 확인 |
...
| **폴더 구조** | 운영 목적에 따라 인프라, 서비스, 보안, 비즈니스, AI, 알림 화면을 분리 |
...
Trace 탐색 화면에서 서비스 호출 순서와 지연 구간을 확인하고, 느린 요청이나 병목 구간을 추적합니다. 대시보드와 로그 화면에서 확인한 요청을 Trace 기준으로 이어서 분석할 때 사용합니다.
...
| **비즈니스 지표 확인** | 비즈니스 관측 | 인증, 대기열, 좌석 Hold, 주문 결제 전환 확인 |
