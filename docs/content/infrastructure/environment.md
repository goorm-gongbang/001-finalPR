...
| ----------- | ----------------------------------------------- | -------------------------- | ------------------------- | -------------------------- |
| **Dev**     | kubeadm (2-node)                                | Cloudflare + Istio Gateway | PostgreSQL Pod, Redis Pod | 기능 개발, 초기 통합 검증  |
...
> Staging은 **Spot 중심**으로 비용을 최적화하면서 중단 내성을 상시 검증하고, Prod는 **On-Demand**로 안정성을 우선합니다. ([Staging Spot 다양화 정책·조치 이력](./operational-troubleshooting/staging-spot) 참조)
