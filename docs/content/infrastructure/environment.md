...
Playball은 Dev, Staging, Prod 세 가지 환경을 목적에 따라 분리하여 운영합니다. Dev에서 기능을 개발하고, Staging에서 Prod와 동일한 구성으로 검증한 뒤, Prod에 반영합니다.
...
| 환경 | 인프라 | 목적 |
|---|---|---|
| **Dev** | kubeadm (MiniPC 2대) | 서비스 개발, 기능 테스트 |
| **Staging** | AWS EKS | QA, 부하테스트, 보안테스트 |
| **Prod** | AWS EKS (고가용성) | 실제 운영 |
...
Prod는 실제 서비스를 위한 AWS EKS 환경입니다. Multi-AZ를 기본으로 하고, RDS/Redis 고가용성 구성, GitOps 배포, 모니터링/알람, 백업/복구 기준을 함께 운영합니다.
