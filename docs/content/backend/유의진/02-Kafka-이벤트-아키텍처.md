...
- Order-Core: 결제/취소/입금만료 이벤트 발행
- Auth-Guard: 유저 차단 이벤트 발행
- 커밋 후 발행 패턴 적용

3. 컨슈머

- Seat: 좌석 상태 전이 소비자 3개
- Order-Core: 이메일 소비자 2개 + 유저차단 소비자 1개
...
- 서비스 간 책임 분리
- 커밋 후 이벤트 발행으로 정합성 개선
- 소비 실패 격리(DLT)로 장애 전파 완화
- Seat 컨슈머의 상태전이 기반 중복 내성

### 한계/주의

- DLT 재처리 워커는 현재 별도 구현되어 있지 않음
- 이메일 소비는 강한 dedupe 저장소가 없어 중복 발송 가능성 이론적으로 존재
- `user-blocked.DLT` 토픽은 공통 토픽 생성 Bean에 현재 명시되어 있지 않음
  (운영 환경의 토픽 자동 생성 정책에 따라 별도 관리 필요)
...
- "소비자 내 상태전이 기반 안전성"
  을 조합해,
  티켓팅 도메인의 고빈도 후속 처리(좌석 반영/이메일/차단 후속조치)를 안정적으로 운영하는 구조다.

특히 컨슈머 보장 전략의 핵심은
**at-least-once를 전제로 실패를 격리하고, 소비 로직을 멱등/준멱등하게 설계해 실질 안정성을 확보하는 것**이다.
