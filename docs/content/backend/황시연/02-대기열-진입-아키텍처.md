...
flowchart LR
    ...
    Q -- "READY + Set-Cookie(admissionToken)" --> C
    C -- "admissionToken Cookie" --> G
    G --> S[Seat Service]
    S --> V[Admission Token Validator]
```
...
stateDiagram-v2
    [*] --> WAITING: enter
    WAITING --> READY: scheduler promote (1s, batch<=100)
    READY --> EXPIRED: ready TTL 만료
    READY --> SEAT_ENTRY: admissionToken으로 좌석 API 접근
    WAITING --> LEFT: leave/reenter
    READY --> LEFT: leave
    EXPIRED --> [*]
    LEFT --> [*]
    SEAT_ENTRY --> [*]
